...
- The single source of truth is `schema.json`. If anything in this document 
...
`schema.json` and written to the graph before the next turn begins. 
...
| `schema.json` | JSON | Hydra `GraphSchema` — single source of truth for all vertex/edge labels and property types | 

- | `section_map.json` | JSON | Maps the 7 interview sections to target vertex types, edge patterns, and extraction goals | 

- | `provenance_spec.json` | JSON | ProvenanceEvidence field 
...
- | `validation_rules.json` | JSON | 15 validation rules (13 hard, 2 soft) enforced on every delta before write | 

- | `session_prompt.txt` | TXT | Full 7-section expert interview prompt with global deep-dive rule | 
...
**ID convention:** `session:hypertension:{YYYY-MM-DD}` **Example:** `session:hypertension:2026-06-18` 
...
**ID convention:** `section:{session_id}:{order}` **Example:** `section:session:hypertension:2026-06-18:2` 
...
**ID convention:** `ep:{session_id}:{sequence}` **Example:** `ep:session:hypertension:2026-06-18:7` 
...
knowledge vertex must have at least one `supportedBy` or `conceptSupportedBy` edge pointing to a ProvenanceEvidence node. 
...
**ID convention:** `prov:{episode_id}:{sequence}` **Example:** `prov:ep:session:hypertension:2026-06-18:7:01` 
...
- `| `supportedBy` | (any knowledge vertex) → ProvenanceEvidence | Traces the knowledge node to its transcript source |` 
...
3. **No invented labels.** Only vertex and edge labels declared in `schema.json` are valid. Any unknown label causes immediate delta rejection. 
...
6. **Provenance on every knowledge node.** Every Layer 2–4 vertex must have a `supportedBy` or `conceptSupportedBy` edge to a `ProvenanceEvidence` node in the same delta. 
...
- | `KnowledgeSession` | `session:hypertension:{YYYY-MM-DD}` | 

`session:hypertension:2026-06-18` | 

- | `SessionSection` | `section:{session_id}:{order}` | 

`section:session:hypertension:2026-06-18:2` | 

- | `TranscriptEpisode` | `ep:{session_id}:{sequence}` | 

`ep:session:hypertension:2026-06-18:7` | 
...
- `prov:ep:session:hypertension:2026-06-18:7:01` | 
